...
var CONFIG = {
  ...
  MEMBERS_DRIVE_API_KEY:           "AIzaSyDf5tLVz5ciYtGLpp67xB1tFEudFB2I0cI",
  //
...
(function(){
  ...
  // PDFs + Word docs + slide decks + videos in a folder, newest first
...
var PER_PAGE = 16;   // 4 rows × 4 columns
  ...
  function renderDocs(files){
    ...
    files.forEach(function(f){
      ...
      var isSlides = ((f.mimeType||'').indexOf('presentation')!==-1 || (f.mimeType||'').indexOf('powerpoint')!==-1);
      var type = (f.mimeType==='application/pdf') ? 'PDF' : (isVideo ? 'VIDEO' : (isSlides ? 'PPT' : 'DOC'));
      var a=document.createElement('a');
      a.className='member-doc';
      a.setAttribute('data-type', type);
      a.setAttribute('role','listitem');
      // videos + slide decks open in the Drive viewer; papers download directly
      a.href = (isVideo || isSlides)
        ? 'https://drive.google.com/file/d/' + f.id + '/view'
        : 'https://drive.google.com/uc?export=download&id=' + f.id;
      a.target='_blank'; a.rel='noopener';
      a.setAttribute('aria-label', ((isVideo||isSlides)?'View ':'Download ')+(f.name||'document'));
  ...
  function cleanName(n){
    if(!n) return '';
    return n.replace(/\.(jpe?g|png|gif|webp|heic|pdf|docx?|pptx?|mp4|mov|m4v|)$/i,'').replace(/[_-]+/g,' ').trim();
  }
